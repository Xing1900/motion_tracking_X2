...
def main(cfg: DictConfig):
    ...
    start_iter = cfg.get("checkpoint_resume_iter", cfg.get("start_iter", 0))
    ...
    if need_logging:
        ...
        def save(policy, checkpoint_name: str, iter: int = None, env_frames: int = None):
            ...
            state_dict["cfg"] = cfg
            if iter is not None:
                state_dict["iter"] = int(iter)
            if env_frames is not None:
                state_dict["env_frames"] = int(env_frames)
    ...
    env_frames = cfg.get("checkpoint_resume_env_frames", 0)
    # Fallback for old checkpoints without env_frames: estimate from start_iter.
    if env_frames == 0 and start_iter > 0:
        env_frames = start_iter * frames_per_batch
    carry = env.reset()

    assert env.training
    remaining_iters = total_iters - start_iter
    if aa.is_main_process():
        progress = tqdm(range(remaining_iters))
    else:
        progress = range(remaining_iters)
    ...
    for i in progress:
        ...
        if hasattr(policy, "step_schedule"):
            policy.step_schedule(global_i / total_iters, global_i)
        if hasattr(env, "step_schedule"):
            env.step_schedule(global_i / total_iters, global_i)
        ...
        if need_logging:
            ...
            if save_interval and save_interval > 0 and should_save(global_i):
                save(policy, f"checkpoint_{global_i}", iter=global_i, env_frames=env_frames)

            run.log(info, step=global_i)
    ...
    if aa.is_main_process():
        ...
        if need_logging:
            save(policy, "checkpoint_final", iter=total_iters, env_frames=env_frames)
